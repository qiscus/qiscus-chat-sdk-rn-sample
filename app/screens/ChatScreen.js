...
import getFileType, {getMime} from 'utils/getFileType';
...
export default class ChatScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    this.subscription1 = Qiscus.newMessage$()
      .subscribe({
        next: (message) => this._onNewMessage(message)
      });
    this.subscription2 = Qiscus.messageRead$()
      .subscribe({
        next: (data) => this._onMessageRead(data)
      });
    this.subscription3 = Qiscus.messageDelivered$()
      .subscribe({
  ...
  _submitMessage = async (text) => {
    ...
    const resp = await Qiscus.qiscus.sendComment(this.state.room.id, text, message.unique_temp_id);
  ...
  _addMessage = (message, scroll = false) => new Promise((resolve) => {
    this.setState((state) => ({
      messages: {
        ...state.messages,
        [message.unique_temp_id]: message,
  ...
  _loadMore = () => {
    ...
    Qiscus.qiscus.loadComments(roomId, {last_comment_id: lastCommentId})
      .then((messages) => {
        ...
        this.setState((state) => ({
          messages: {
            ...state.messages,
            ...messages.reduce((result, item) => (result[item.unique_temp_id] = item, result), {})
          },
          isLoadMoreable,
        }));
      })
  ...
  _sortMessage = (messages) => messages.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
