...
export default class RoomListScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    this.subscription = Qiscus.newMessage$()
      .subscribe({
        next: (message) => {
  ...
  render() {
    const avatarURL = this.state.avatarURI != null
    ...
    return (
      <View style={styles.container}>
        <Toolbar
          ...
          renderRightButton={() => (
            <TouchableOpacity style={styles.btnAvatar}
              ...
              <Image style={styles.iconStartChat}
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <RoomItem room={item} onClick={(roomId) => this._onClickRoom(roomId)}/>
          )}/>
      </View>
    )
  }
