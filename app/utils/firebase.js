...
export const initiate$ = () => Qiscus.isLogin$()
  .filter(it => it === true)
  .map(() => getToken$()).flatten()
  .take(1)
...
export const onNotification$ = () => xs.create({
  listener: null,
  start(listener) {
...
export const onNotificationDisplayed$ = () => xs.create({
  listener: null,
  start(listener) {
    this.listener = firebase.notifications().onNotificationDisplayed((notification) => {
      listener.next(notification)
    })
  },
  stop() {
...
export const onNotificationOpened$ = () => xs.create({
  listener: null,
  start(listener) {
    this.listener = firebase.notifications().onNotificationOpened((notification) => {
      listener.next(notification)
    })
  },
  stop() {
...
export const displayNotification = (notification) => firebase
  .notifications()
  .displayNotification(notification)
export const getInitialNotification = () => firebase.notifications()
...
export function start(onDestroy) {
  ...
  const listener2 = firebase.notifications()
  ...
  const listener3 = firebase.notifications()
